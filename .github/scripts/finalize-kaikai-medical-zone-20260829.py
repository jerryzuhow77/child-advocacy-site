...
def ensure_witness_entry(path: Path, section_id: str, href: str, label: str) -> None:
    ...
    # Earlier one-off attempts placed this CTA in more than one location. Remove
    # every anchor whose visible label matches, regardless of class or attribute
    # order, then remove any empty wrapper left behind.
    labeled_anchor = re.compile(
        rf'<a\b[^>]*>\s*{re.escape(label)}\s*</a>',
        re.S,
    )
    text = labeled_anchor.sub('', text)
    empty_medical_wrapper = re.compile(
        r'<div\s+class="[^"]*(?:official-source-links\s+medical-zone-entry|medical-zone-entry\s+official-source-links)[^"]*">\s*</div>',
        re.S,
    )
    text = empty_medical_wrapper.sub('', text)
    ...
    visible_label_links = re.findall(
        rf'<a\b[^>]*>\s*{re.escape(label)}\s*</a>',
        text,
        flags=re.S,
    )
    if len(canonical_links) != 1 or len(visible_label_links) != 1:
        raise SystemExit(
            f'{path}: expected one canonical medical-zone anchor; '
            f'canonical={len(canonical_links)}, all-label-links={len(visible_label_links)}'
        )
