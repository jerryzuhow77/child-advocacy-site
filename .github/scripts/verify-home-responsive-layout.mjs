...
try {
  for (const width of widths) {
    ...
    assert(initial.counts.pinned === 32, `[${width}] 置頂入口數量應為 32`, initial.counts.pinned);
    ...
    try {
      ...
      if ([390, 761, 1180].includes(width)) {
        ...
        await captureSection(page, '#chen-shangjie-high-court-action-20260916', path.join(outputDir, `memorial-${width}.png`));
