...
const output = 'browser-validation/kaikai-chapter-two-major-live';
...
for (const [language, site] of Object.entries(sites)) {
  for (const [viewportName, viewport] of Object.entries(viewports)) {
    ...
    try {
      ...
      const response = await page.goto(`${site.url}${separator}qa=${Date.now()}-${language}-${viewportName}`, {
      ...
      result.state = await page.evaluate((expected) => {
        ...
        const chapterText = chapterGroup?.textContent || '';
        const chapterLinks = [...(chapterGroup?.querySelectorAll('.special-feature-prologue-children a[href]') || [])];
        const chapterButton = chapterGroup?.querySelector(':scope > button');
        ...
        return {
          ...
          launchActionCount: launch?.querySelectorAll('.home-chapter-two-launch-actions a[href]').length || 0,
          ...
          groupControl: chapterButton?.getAttribute('aria-controls') || '',
          groupHrefs: chapterLinks.map((link) => link.href),
          menuTitle: chapterText.includes(expected.menuTitle),
          fiveMinute: chapterText.includes(expected.fiveMinute),
          outOfHome: chapterText.includes(expected.outOfHome),
          harmCases: chapterText.includes(expected.harmCases),
          responsibility: chapterText.includes(expected.responsibility),
          witnesses: chapterText.includes(expected.witnesses),
          ...
          legacyCopyAbsent: !document.body.textContent.includes('悲劇不忘・終章｜剴剴的672天') && !document.body.textContent.includes('悲剧不忘・终章｜剀剀的672天'),
          cardHref: card?.href || '',
      ...
      result.target = {
        ...
        witnesses: targetHtml.includes('witnesses/'),
      ...
      result.checks = {
        ...
        uniqueChapterGroup: result.state.groupCount === 1 && result.state.groupLinkCount === 6 && result.state.groupControl === 'specialFeatureChapterTwoZh',
        ...
        chapterOnePreserved: result.state.chapterOneGroupCount === 1,
        promotedCard: result.state.cardCount === 1 && result.state.cardTitle && result.state.cardAction && result.state.legacyCopyAbsent && result.state.cardHref === site.chapterUrl,
        ...
        targetIsFullChapter: result.target.chapterBrief && result.target.placement && result.target.harmCases && result.target.system && result.target.witnesses && result.target.title,
...
const lines = ['# Kaikai Chapter 2 major homepage live browser verification', ''];
for (const result of results) {
  ...
  lines.push(`- Menu: links=${result.state?.groupLinkCount ?? 'n/a'}, expanded=${result.menu?.expanded ?? 'n/a'}, child=${result.menu?.childId ?? 'n/a'}`);
