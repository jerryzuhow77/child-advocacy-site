(() => {
  ...
  function buildVisual(text, roots) {
    const visual=document.createElement('div'); visual.className='home-media-visual'; visual.setAttribute('role','img'); visual.setAttribute('aria-label',text.art); visual.style.backgroundImage=`url("${roots.art}")`; visual.style.backgroundSize='cover'; visual.style.backgroundPosition='center';
    const art=document.createElement('img'); art.className='home-media-pastel-art'; art.src=roots.art; art.alt=''; art.loading='eager'; art.fetchPriority='high'; art.decoding='async'; art.style.objectPosition='center';
