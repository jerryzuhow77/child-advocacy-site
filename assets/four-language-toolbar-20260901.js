(() => {
  ...
  async function recordOrReadView(route,element){if(!route||document.querySelector("[data-home-view-counter]"))return null;const key=articleKey(route),seen=`cpa_article_viewed_${key}`;let inc=true;try{inc=!sessionStorage.getItem(seen)}catch(_){};try{if(inc)sessionStorage.setItem(seen,"pending");const data=await request(key,{action:inc?"view":"read",clientId:clientId()});const current=Number(data.viewCount);if(!Number.isFinite(current)||current<0)throw new Error("invalid counter");if(inc)sessionStorage.setItem(seen,"1");const value=Math.max(current,await historicalView(route,key));element.querySelector("b").textContent=format(value);element.hidden=false;return {...data,viewCount:value}}catch(_){try{if(inc)sessionStorage.removeItem(seen)}catch(_){}}try{const value=await historicalView(route,key);element.querySelector("b").textContent=format(value);element.hidden=false;return {viewCount:value}}catch(_){}return null}
