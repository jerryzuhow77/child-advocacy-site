(function(){
  ...
  var VERSION='20260823-relief-map-1';
  ...
  var words={
    'zh-Hant':{alt:'東亞案件記憶地圖，以立體多層紙雕呈現大陸、台灣、香港、日本、韓國與周邊海域',guide:'點選地區圖章，展開案件年代與檔案',label:'東亞案件記憶地圖地區選擇'},
    'zh-Hans':{alt:'东亚案件记忆地图，以立体多层纸雕呈现大陆、台湾、香港、日本、韩国与周边海域',guide:'点击地区图章，展开案件年代与档案',label:'东亚案件记忆地图地区选择'},
    en:{alt:'East Asia case memory map rendered as an intricate layered paper-cut relief',guide:'Select a regional seal to open its timeline and case archive',label:'Choose a region on the East Asia case memory map'},
    ja:{alt:'東アジア事件記憶地図を精緻な多層切り絵レリーフで表現',guide:'地域の印章を選び、年代と事件資料を開きます',label:'東アジア事件記憶地図の地域選択'}
  };
  ...
  function enhance(){
    ...
    if(window.gsap&&!reduce){
      window.gsap.fromTo(figure,{y:24,scale:.965,autoAlpha:0},{y:0,scale:1,autoAlpha:1,duration:.85,ease:'power3.out',scrollTrigger:{trigger:figure,start:'top 90%',once:true}});
      window.gsap.fromTo(buttons,{y:12,scale:.82,autoAlpha:0},{y:0,scale:1,autoAlpha:1,duration:.48,stagger:.055,ease:'back.out(1.6)',scrollTrigger:{trigger:figure,start:'top 86%',once:true}});
      ...
      if(image&&window.matchMedia('(hover:hover) and (pointer:fine)').matches){
        figure.addEventListener('pointermove',function(event){
          ...
          window.gsap.to(image,{xPercent:x*1.6,yPercent:y*1.2,scale:1.025,duration:.55,ease:'power2.out',overwrite:true});
        });
        figure.addEventListener('pointerleave',function(){window.gsap.to(image,{xPercent:0,yPercent:0,scale:1,duration:.65,ease:'power2.out',overwrite:true});});
      }
    }
  }
