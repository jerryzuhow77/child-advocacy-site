(() => {
  ...
  const searchCopy = searchLocale === 'zh-Hans'
    ...
    : { idle: "Enter a keyword to search all of Chapter Two, including collapsed records.", none: "No results for “{query}”", count: "<strong>{count}</strong> results for “{query}”; currently <strong>{current}</strong>.", capped: "More than 500 results. Please use a more specific keyword." };
  let searchMarks = [];
  let searchCurrent = -1;
  let searchTimer = 0;
  ...
  const prologueAudioPrompt = locale === 'zh-Hans' ? "Play prologue and music" : "Play prologue and music";
  ...
  const totalLabel = locale === 'zh-Hans' ? "life milestones" : "life milestones";
  const resultLabel = locale === 'zh-Hans' ? "matching milestones" : "matching milestones";
  ...
  if (lifeStatus) {
    ...
    lifeStatus.textContent = query
      ? `${locale === 'zh-Hans' ? "Found" : "Found"} ${shown} ${resultLabel}`
      : quickCollapsed
        ? `${locale === 'zh-Hans' ? "Highlights" : "Highlights"} ${quickShown} / ${lifeEvents.length} ${totalLabel}`
        : `${locale === 'zh-Hans' ? "Showing" : "Showing"} ${lifeEvents.length} ${totalLabel}`;
  }
};
...
lifeSearch?.addEventListener('input', filterLifeEvents);
  ...
  const readingDepthCopy = locale === 'zh-Hans'
    ? {
        quick: "Five-minute mode: key conclusions and three evidence-status levels.",
        guided: "Fifteen-minute mode: people, organizations, medical warnings and responsibility follow-through.",
        full: "Full-record mode: also show and expand the DAY1–DAY10 daily record links."
      }
    : {
        quick: "Five-minute mode: key conclusions and three evidence-status levels.",
        guided: "Fifteen-minute mode: people, organizations, medical warnings and responsibility follow-through.",
...
const audioCopy = locale === 'zh-Hans'
  ? { play: "Play music", pause: "Pause music", replay: "Replay music", track: '当前曲目' }
  : { play: "Play music", pause: "Pause music", replay: "Replay music", track: "Current track" };
...
const audioTracks = [
  {
    title: locale === 'zh-Hans' ? "Unspoken Notes" : "Unspoken Notes",
...
if (audio && audioTracks.length) {
  ...
  const audioPanelCopy = locale === 'zh-Hans' ? { expand: "Expand music controls", collapse: "Collapse music controls" } : { expand: "Expand music controls", collapse: "Collapse music controls" };
  ...
  const initPuppetTheatre = () => {
    ...
    const theatreCopy = locale === 'zh-Hans'
      ? { play: "Start performance", continue: "Continue", playing: "Playing", replay: "Watch again", reduced: "Reduced-motion mode enabled", unavailable: "Animation unavailable" }
      : { play: "Start performance", continue: "Continue", playing: "Playing", replay: "Watch again", reduced: "Reduced-motion mode enabled", unavailable: "Animation unavailable" };

    const showDialogue = (index) => {
      const line = dialogueLines[index];
      if (!line || !dialogueSpeaker || !dialogueCopy) return;
      dialogueSpeaker.textContent = ({"女": "Woman", "男": "Man", "同": "Together", "男女同": "Together"})[line.speaker] || line.speaker;
...
(() => {
  ...
  const copy = isHans
    ? {
        ...
        exportDone: "Source index downloaded", showLife: "Show all life milestones", hideLife: "Collapse to five-minute highlights",
        top: "Back to top", revealed: '已为这个深链接暂时展开完整段落。', exportName: "kaikai-chapter-two-sources.tsv",
        shareText: "Kaikai special report, Chapter Two: a five-minute introduction to two adoption processes, breaks in the response to warning signs and institutional responsibilities."
      }
    : {
        sectionLink: "Copy section link", copied: "Link copied", shareDone: "Brief link copied",
        exportDone: "Source index downloaded", showLife: "Show all life milestones", hideLife: "Collapse to five-minute highlights",
        top: "Back to top", revealed: "The full section has been temporarily expanded for this direct link.", exportName: "kaikai-chapter-two-sources.tsv",
        shareText: "Kaikai special report, Chapter Two: a five-minute introduction to two adoption processes, breaks in the response to warning signs and institutional responsibilities."
      };
...
const applyReadingDepth = (scrollToHash = false) => {
  ...
  document.querySelectorAll('main section[id]').forEach((section) => {
    ...
    link.textContent = isHans ? '链接' : "Link";
  ...
  document.querySelector('[data-export-sources]')?.addEventListener('click', () => {
    const rows = [[isHans ? "No." : "No.", isHans ? "Source" : "Source", isHans ? "URL" : "URL"]];
