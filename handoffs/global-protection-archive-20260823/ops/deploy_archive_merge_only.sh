...
The script builds the app but deliberately does not restart production services;
restart/release must use the host's existing deployment mechanism after review.
EOF
}
...
if [[ -n "$(git status --porcelain)" ]]; then
  echo 'Abort: production worktree has uncommitted changes.' >&2
  exit 4
fi
...
echo 'Checking component patch...'
patch --dry-run -p1 < "$COMPONENT_PATCH"
echo 'Checking CSS patch...'
...
[[ "${CONFIRM_GLOBAL_PROTECTION_DEPLOY:-}" == 'YES' ]] || {
  echo 'Abort: set CONFIRM_GLOBAL_PROTECTION_DEPLOY=YES for execute mode.' >&2
  exit 5
}
...
rollback() {
  rc=$?
  if ((rc != 0)); then
    echo "Failure detected; resetting feature branch to the backup commit." >&2
...
if [[ -f package.json ]]; then
  ...
  echo 'No package.json found; production build cannot be verified.' >&2
  exit 6
fi
...
echo "Build passed and commit created on $FEATURE_BRANCH."
echo "Backup branch: $BACKUP_BRANCH"
echo 'No production service was restarted. Use the host’s existing release procedure only after full regression testing.'
