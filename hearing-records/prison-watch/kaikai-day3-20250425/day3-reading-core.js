(() => {
  ...
  const progress = document.querySelector('.day3-progress i');
  ...
  const fullRecord = document.querySelector('.day3-full-record');
  const transcript = fullRecord?.querySelector('.day3-transcript');
  const chapters = transcript ? [...transcript.querySelectorAll('.day3-transcript-chapter')] : [];
  let readerBar;
  let readerCount;
  let reader;
  let toggleAll;
  ...
  const setChapterOpen = (chapter, open, animate = true) => {
    const flow = chapter.querySelector('.day3-dialogue-flow');
    const button = chapter.querySelector('.day3-chapter-toggle');
  ...
  if (fullRecord && transcript && chapters.length) {
    ...
    ambience.className = 'day3-record-ambience';
    ambience.setAttribute('aria-hidden', 'true');
    ambience.innerHTML = '<i></i><i></i><i></i>';
    fullRecord.prepend(ambience);

    const tools = document.createElement('section');
    tools.className = 'day3-record-tools';
    tools.setAttribute('aria-label', t.toolsLabel);
    tools.innerHTML = `
      <div class="day3-record-tools-head">
        <div class="day3-record-tools-copy">
          <strong>${t.procedure}</strong>
          <span>${t.procedureNote}</span>
        </div>
        <button class="day3-record-toggle-all" type="button">${t.expandAll}</button>
      </div>
      <nav class="day3-chapter-jumps" aria-label="${t.jumps}"></nav>`;
    toggleAll = tools.querySelector('.day3-record-toggle-all');
    const jumps = tools.querySelector('.day3-chapter-jumps');

    reader = document.createElement('div');
    reader.className = 'day3-record-progress';
    ...
    chapters.forEach((chapter, index) => {
      ...
      button.className = 'day3-chapter-toggle';
      button.type = 'button';
  ...
  gsap.from('.day3-remembrance span', {
    y: 22,
    rotate: -5,
    scale: .82,
    autoAlpha: 0,
  ...
  gsap.from('.day3-hero-poster', { x: 55, rotate: 5, autoAlpha: 0, duration: 1.15, ease: 'power3.out' });
  gsap.to('.day3-hero-paper', {
    yPercent: 12,
    ease: 'none',
    scrollTrigger: { trigger: '.day3-hero', start: 'top top', end: 'bottom top', scrub: 1 }
  });

  gsap.utils.toArray('.day3-reveal')
    .filter(element => !element.closest('.day3-juror-wall, .day3-lens-grid, .day3-full-record'))
  ...
  gsap.utils.toArray('.day3-lens-grid article').forEach((card, index) => {
    gsap.fromTo(
      card,
  ...
  const jurorCards = gsap.utils.toArray('.day3-juror-wall article');
  const jurorPath = document.querySelector('.day3-question-path');
  ...
  gsap.to('.day3-closing-door i:first-child', {
    xPercent: -18,
    ease: 'none',
    scrollTrigger: { trigger: '.day3-closing', start: 'top 70%', end: 'center 45%', scrub: 1 }
  });
  gsap.to('.day3-closing-door i:last-child', {
    xPercent: 18,
    ease: 'none',
    scrollTrigger: { trigger: '.day3-closing', start: 'top 70%', end: 'center 45%', scrub: 1 }
  });
  ...
  if (fullRecord && chapters.length) {
    ...
    gsap.to('.day3-pdf-figure img', {
      scale: 1.045,
      yPercent: 3,
      ease: 'none',
      scrollTrigger: { trigger: '.day3-pdf-figure', start: 'top 85%', end: 'bottom 20%', scrub: 1 }
    });
    gsap.to('.day3-record-ambience i:nth-child(1)', {
      y: 150,
      rotate: 24,
      ease: 'none',
      scrollTrigger: { trigger: fullRecord, start: 'top bottom', end: 'bottom top', scrub: 1.4 }
    });
    gsap.to('.day3-record-ambience i:nth-child(2)', {
      y: -130,
      rotate: -16,
      ease: 'none',
      scrollTrigger: { trigger: fullRecord, start: 'top bottom', end: 'bottom top', scrub: 1.6 }
    });
    gsap.to('.day3-record-ambience i:nth-child(3)', {
      y: -90,
      scale: 1.16,
      ease: 'none',
