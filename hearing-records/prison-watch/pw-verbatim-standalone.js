(() => {
  ...
  const questionerFrom = (value) => {
    const match = tidy(value).match(/(?:^|[。；;])\s*((?:檢察官|检察官|審判長|审判长|陪席法官|受命法官|國民法官(?:（[^）]+）)?|国民法官(?:（[^）]+）)?|[^。；;]{1,24}?辯護人|[^。；;]{1,24}?辩护人|Prosecutor|Presiding Judge|Associate Judge|Commissioned Judge|Citizen Judge(?:\s*\([^)]*\))?|[^.;]{1,40}?counsel|検察官|裁判長|陪席裁判官|受命裁判官|国民裁判官(?:（[^）]+）)?|[^。；;]{1,24}?弁護人)(?:主詰問|反詰問|覆主詰問|詢問|询问|質問|尋問| examination| asks?))/i);
  ...
  const labelSpeakers = (section) => {
    ...
    section.querySelectorAll('.pw-verbatim-rows').forEach((group) => {
      ...
      group.querySelectorAll(':scope > .pw-verbatim-row').forEach((row) => {
        ...
        const nextRespondent = respondentFrom(right.querySelector('p')?.innerText || '');
