(() => {
  ...
  const searchCopy = searchLocale === 'zh-Hans'
    ...
    : { idle: "キーワードを入力すると、折りたたまれた記録を含む第2章全文を検索できます。", none: "「{query}」は見つかりませんでした", count: "「{query}」は<strong>{count}</strong>件。現在<strong>{current}</strong>件目です。", capped: "500件を超える結果があります。より具体的なキーワードを入力してください。" };
  let searchMarks = [];
  let searchCurrent = -1;
  let searchTimer = 0;
  ...
  const prologueAudioPrompt = locale === 'zh-Hans' ? "序幕と音楽を再生" : "序幕と音楽を再生";
  ...
  const totalLabel = locale === 'zh-Hans' ? "件の人生の節目" : "件の人生の節目";
  const resultLabel = locale === 'zh-Hans' ? "件の一致する節目" : "件の一致する節目";
  ...
  if (lifeStatus) {
    ...
    lifeStatus.textContent = query
      ? `${locale === 'zh-Hans' ? "検索結果" : "検索結果"} ${shown} ${resultLabel}`
      : quickCollapsed
        ? `${locale === 'zh-Hans' ? "要点表示" : "要点表示"} ${quickShown} / ${lifeEvents.length} ${totalLabel}`
        : `${locale === 'zh-Hans' ? "表示" : "表示"} ${lifeEvents.length} ${totalLabel}`;
  }
};
...
lifeSearch?.addEventListener('input', filterLifeEvents);
  ...
  const readingDepthCopy = locale === 'zh-Hans'
    ? {
        quick: "5分モード：主要な結論と3段階の証拠の状況を表示。",
        guided: "15分モード：人物、機関、医療の警告、責任の連携を表示。",
        full: "全記録モード：DAY1〜DAY10の各期日の入口も表示して展開。"
      }
    : {
        quick: "5分モード：主要な結論と3段階の証拠の状況を表示。",
        guided: "15分モード：人物、機関、医療の警告、責任の連携を表示。",
...
const audioCopy = locale === 'zh-Hans'
  ? { play: "音楽を再生", pause: "音楽を一時停止", replay: "音楽を再再生", track: '当前曲目' }
  : { play: "音楽を再生", pause: "音楽を一時停止", replay: "音楽を再再生", track: "現在の曲" };
...
const audioTracks = [
  {
    title: locale === 'zh-Hans' ? "言葉にならなかった音符" : "言葉にならなかった音符",
...
if (audio && audioTracks.length) {
  ...
  const audioPanelCopy = locale === 'zh-Hans' ? { expand: "音楽の操作パネルを開く", collapse: "音楽の操作パネルを閉じる" } : { expand: "音楽の操作パネルを開く", collapse: "音楽の操作パネルを閉じる" };
  ...
  const initPuppetTheatre = () => {
    ...
    const theatreCopy = locale === 'zh-Hans'
      ? { play: "開演", continue: "続ける", playing: "上演中", replay: "もう一度見る", reduced: "動きを減らすモードを適用済み", unavailable: "アニメーションを利用できません" }
      : { play: "開演", continue: "続ける", playing: "上演中", replay: "もう一度見る", reduced: "動きを減らすモードを適用済み", unavailable: "アニメーションを利用できません" };

    const showDialogue = (index) => {
      const line = dialogueLines[index];
      if (!line || !dialogueSpeaker || !dialogueCopy) return;
      dialogueSpeaker.textContent = ({"女": "女", "男": "男", "同": "一緒に", "男女同": "一緒に"})[line.speaker] || line.speaker;
...
(() => {
  ...
  const copy = isHans
    ? {
        ...
        exportDone: "出典一覧をダウンロードしました", showLife: "人生の節目をすべて表示", hideLife: "5分で読む要点に折りたたむ",
        top: "ページ上部へ", revealed: '已为这个深链接暂时展开完整段落。', exportName: "kaikai-chapter-two-sources-ja.tsv",
        shareText: "カイカイ事件特集第2章：2回の養子縁組手続、警告への対応の断絶、制度上の責任をまず5分で把握。"
      }
    : {
        sectionLink: "この節のリンクをコピー", copied: "リンクをコピーしました", shareDone: "要点版のリンクをコピーしました",
        exportDone: "出典一覧をダウンロードしました", showLife: "人生の節目をすべて表示", hideLife: "5分で読む要点に折りたたむ",
        top: "ページ上部へ", revealed: "この直接リンクのために、該当部分の全文を一時的に展開しました。", exportName: "kaikai-chapter-two-sources-ja.tsv",
        shareText: "カイカイ事件特集第2章：2回の養子縁組手続、警告への対応の断絶、制度上の責任をまず5分で把握。"
      };
...
const applyReadingDepth = (scrollToHash = false) => {
  ...
  document.querySelectorAll('main section[id]').forEach((section) => {
    ...
    link.textContent = isHans ? '链接' : "リンク";
  ...
  document.querySelector('[data-export-sources]')?.addEventListener('click', () => {
    const rows = [[isHans ? "番号" : "番号", isHans ? "出典" : "出典", isHans ? "URL" : "URL"]];
